...
private:
    ...
    long double FDerivative(long double x, long double y)   //Производная
    {
        auto numeric = (F(x, y + H/2.0) - F(x, y - H/2.0)) / H + (F(x + H/2.0, y) - F(x + H/2.0, y)) / H;
        return numeric;
    }
        ...
        #pragma omp parallel sections num_threads(2) if(RunParallel)
        {
            #pragma omp section
            {
                ...
                for (int k = 1; k < N - 1; k++)
                {
                    ...
                    Psi0[k + 1] = (Psi0[k] * deriv + diff) * H * H + 2.0 * Psi0[k] - Psi0[k - 1];
                }
            }
            ...
            #pragma omp section
            {
                ...
                for (int k = 1; k < N - 1; k++)
                {
                    long double deriv = FDerivative(XBand[k], YBand[k]);
                    long double diff = F(XBand[k], YBand[k]) - NumerovApproximation(k);
                    Psi2[k + 1] = (Psi2[k] * deriv + diff) * H * H + 2.0 * Psi2[k] - Psi2[k - 1];
                }
            }
        }
    }
...
public: 
    ...
    {
        H = (XStop - XStart) / ((long double) N);   //задание равномерной сетки
        XBand = new long double[N] { };
        for (int k = 0; k < N; k++)
            XBand[k] = XStart + (XStop - XStart) * k / ((long double) (N - 1));     //задание X
